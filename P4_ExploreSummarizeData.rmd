...
ggplot(data = pld, aes(x = LoanOriginationDate)) +
...
The loans are mostly for 36 months with some loans for 60 months and a samll number of loans are 12 months long.


```{r Borrower APR}
ggplot(data = pldn, aes(x = BorrowerAPR*100)) +
  geom_histogram()
...
ggplot(data = pldn, aes(x = MonthlyLoanPayment)) +
  geom_histogram()
```
...
ggplot(data = pldn, aes(x = StatedMonthlyIncome)) +
  geom_histogram() +
  xlim(0, 10000)
...
```{r DebtToIncomeRatio}
ggplot(data = pldn, aes(x = DebtToIncomeRatio)) +
  geom_histogram() +
  xlim(0, 1)
...
In the dataset only the credit score bucket is reported, with the lower and upper range in separate variables. Here the histogram is for the upper range of the individauls credit score. A few samples with credit score of zero are trimmed from the graph. overall, we see a normal distibution for the credit score. 
...
The graphs above show the loan APR broken into various graphs based on the borrowers credit grade. It is clear that borrower with better credit grade (AA>A>B>C>F>E>HR) are getting loans with lower APR.
...
There are 113937 observations each having 81 variables describing a loan. Some varaibles describe the loan (LoanOriginalAmount, LoanOriginationDate, Term, BorrowerAPR, MonthlyLoanPayment, etc.). There are varaibles describing the borrower condition provided by borrower such as ListingCategory, StatedMonthlyIncome, Occupation, etc. Majority of the variables describe the credit status and history of the borrower (CurrentCreditLines, TotalCreditLinespast7years, DelinquenciesLast7Years, RevolvingCreditBalance, etc.). These variables define the risk associated with the borrower and possbiliy define the credit score (CreditScoreRangeUpper and CreditScoreRangeLower) of the borrower.
...
ggplot(data = subset(pldn, ProsperRating..Alpha. != 'NA'),
  ...
  scale_color_brewer(type = 'seq', palette = 'Spectral',
      guide = guide_legend(title = 'Prosper rating', reverse = TRUE)) +
  xlab('Prosper rating') +
  ylab('Borrower APR') +
